...
const MonthlyIncome = ({ amount, change, percent }) => {
  ...
  const trendData = useMemo(() => {
    const incomes = transactions.filter(tx => tx.amount > 0).sort((a, b) => new Date(a.date) - new Date(b.date));
    let cumulative = 0;
    return incomes.map(tx => {
  ...
  return (
    <GlowCard color="emerald" className="h-full shadow-sm">
